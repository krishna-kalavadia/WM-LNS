"""Solves the discrete multi-objective-optimization path planning via a heuristic Weighted Max (h-WM)"""
import math
import heapq
import time
...
def heuristic_weighted_max_solver(G, start_id, goal_id, weights, budget=None, three_dim=False):
    """
    WM and WM-poly
    NOTE: If budget is none, this solver is exact and optimal for the WM problem
    """
    if three_dim:
        num_obj = len(weights)
    ...
    # Keep track of best current cost vectors and paths leading to each node 
    open_list = {start_id: [(initial_raw_objectives, [start_id])]}

    while heap:
        current_cost, _, current_node, cum_objectives, raw_cumulative_objectives, path = heapq.heappop(heap)

        # Skip states that were pruned from this node's open list
        if (raw_cumulative_objectives, path) not in open_list.get(current_node, []):
            continue
        ...
        for neighbor in G.neighbors(current_node):
            ...
            dominated = False
            paths_to_remove = []
            for (existing_path_raw_objectives, path_to_neighbor) in open_list.get(neighbor, []):
                ...
                if dominates(new_raw_cumulative_objectives, existing_path_raw_objectives):
                    paths_to_remove.append((existing_path_raw_objectives, path_to_neighbor))
            ...
            # Budget Implementation for WM-poly
            # Check our budget
            records = open_list.get(neighbor, [])
            if budget is not None and len(records) >= budget:
                # We already have 'b' paths leading to this node, so skip adding the new "tentative path"
                continue

            # Otherwise, go ahead and record & enqueue
            new_path_cost = max(f_vals) + rho * sum(new_raw_cumulative_objectives)
            open_list.setdefault(neighbor, []).append((new_raw_cumulative_objectives, path + [neighbor]))
            heapq.heappush(heap, (new_path_cost, next(counter), neighbor,
                                new_cumulative_objectives, new_raw_cumulative_objectives, path + [neighbor]))

    print("No path found.")
    return None

def main_heuristic_wm(G, nodes, start_id, goal_id, obstacle_list, weights, budget=None, three_dim=False):
    """
    Run our WM
    """    
    ...
    if budget == None:
        optimal_path = heuristic_weighted_max_solver(G, start_id, goal_id, weights, three_dim=three_dim)
    else:
        optimal_path = heuristic_weighted_max_solver(G, start_id, goal_id, weights, budget=budget, three_dim=three_dim)
    
    cum_weighted_objectives = None
    if optimal_path:
        cum_weighted_objectives = compute_path_objectives(optimal_path, G, weights)
        print(f"Cumulative Objectives for Optimal Path: {cum_weighted_objectives}")
        print(f"Cumulative Weighted Max Cost: {max(cum_weighted_objectives)}")
    else:
        print("No optimal path could be found.")
    
    return G, nodes, obstacle_list, start_id, goal_id, optimal_path, cum_weighted_objectives
